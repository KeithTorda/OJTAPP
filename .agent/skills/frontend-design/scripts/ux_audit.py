...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        has_long_text = bool(re.search(r'<p\b|<article\b|<span[^>]*class=[^>]*(?:text|prose)', content, re.IGNORECASE))
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'stitch', 'api', '.agent'}]
            for file in files:
